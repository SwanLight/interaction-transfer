...
| 3 | `log/decisions.md` | **39 条技术决策及其被否决的备选**。改任何设计前先查这里，否则会重新提出已经被否决过的方案 |
| 4 | `log/pitfalls.md` | 35 条坑。**第 3 节「已知会静默失败的坑」尤其重要**，那些不报错、只让结论作废 |
...
out/
  ...
  s2_expert/       训练好的策略录像
  s3_source/       双板 source 示教录像 + 接触部位分布 + 数据集验收
```
...
1. **预训练物体集的采集脚本**。D-39 已把物体集从 4 个补到 6 个并**冻结**
   覆盖论证（`plan/03` §2.4），六个资产已生成并通过 S1 自检 12 项，
   **缺的是阶段机**。这一块直接决定 Gate E 能不能归因，不能省。
