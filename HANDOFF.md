...
| 3 | `log/decisions.md` | **43 条技术决策及其被否决的备选**。改任何设计前先查这里，否则会重新提出已经被否决过的方案 |
| 4 | `log/pitfalls.md` | 39 条坑。**第 3 节「已知会静默失败的坑」尤其重要**，那些不报错、只让结论作废 |
...
| S3 | 双板采集器模拟人，产生多样示教 | 🟡 抽屉 ✅，擦拭/旋钮/预训练集待做 |
| S4 | 从示教里提取交互信息 | ⬜ |
| S4.5 | 表示分辨率该定多少 | ⬜ |
| S5 | 从多条示教归纳出共享的交互要求 | ⬜ |
| S6 | 训练「实现任意交互规格」的执行器 | ⬜ |
...
```
out/
  report.html      总报告，双击就能看（视频内嵌，单文件）
  s0_pipeline/     可视化管线
...
| **S3 Source** | 🟡 **进行中** | **抽屉已完成**（800 条 / 752 成功），其余见下 |
| S4 及之后 | ⬜ 未开始 | |
...
- 独立验收 14 项全通过（`tools/s3_verify_dataset.py`）
- 录像与摘要在 `out/s3_source/`，五个家族各一条

数据集在服务器 `/tmp/s3_drawer_full`（36 MB），按 D-23 不进版本控制。

**已完成 —— 探针物体集（交互原语库），8040 条 / 7023 成功（87.4%）**
...
9 个物体、15 条原语，**10 条同时满足"≥200 条成功"与"≥2 个承载物体"**。
未满足的五条（P7 roll 完全没做成、P8 twist、P3/P13/P14 各只有一个物体）
连同原因写在 `plan/03` §2.4.5 与 D-43，**没有从分类学里删掉**——
删掉等于把分类学改成"我们做得到的那些"。

**未完成 —— S3 的其余三块**

1. **补齐那五条原语格**，尤其 P7 rolling contact（现为 0 个物体）。
2. **擦拭 source**（主任务，`plan/01` §0 的顺序里排在抽屉之后）
3. **旋钮 source**
4. 多样性验收：策略分类器在原始 source 动作上的准确率（`plan/03` §4）
