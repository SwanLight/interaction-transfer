...
| 3 | `log/decisions.md` | **45 条技术决策及其被否决的备选**。改任何设计前先查这里，否则会重新提出已经被否决过的方案 |
| 4 | `log/pitfalls.md` | 41 条坑。**第 3 节「已知会静默失败的坑」尤其重要**，那些不报错、只让结论作废 |
...
| S3 | 双板采集器模拟人，产生多样示教 | 🟡 抽屉/探针集/擦拭/多样性 ✅，旋钮未完成 |
| S4 | 从示教里提取交互信息 | ⬜ |
| S4.5 | 表示分辨率该定多少 | ⬜ |
| S5 | 从多条示教归纳出共享的交互要求 | ⬜ |
| S6 | 训练「实现任意交互规格」的执行器 | ⬜ |
...
| **S3 Source** | 🟡 **进行中** | 抽屉 ✅ 探针集 ✅ **擦拭 ✅** 多样性 ✅，**旋钮未完成** |
| S4 及之后 | ⬜ 未开始 | |
...
1. **旋钮 source**（D-45）：脚本已写完能跑，但五个家族只有 `pin_push_dual`
   稳定通过。已查明并修掉四个真 bug（两块板被指到同一点 ×2、开环圆弧导致
   内力累积、对捏时跟随板变刹车），**仍有一个未解释的现象**：位置控制的板压在
   刚体上时实际接触力远高于"板刚度 × 指令干涉"，而 `pin_push_dual` 又与该估计
   吻合。下一步应先用探针脚本量"干涉 → 接触力"的曲线，**不要继续调增益**。
   已经站得住的一条：`rim_only` 在设计转速下只转到 0.003~0.117 rad
   （目标 1.0~2.2），这是 D-14 的**操作级**证据。
...
tools/
  ...
  s3_source_knob.py       旋钮采集。⚠️ 未完成，见 D-45
