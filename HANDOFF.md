...
| 3 | `log/decisions.md` | **47 条技术决策及其被否决的备选**。改任何设计前先查这里，否则会重新提出已经被否决过的方案 |
| 4 | `log/pitfalls.md` | 48 条坑。**第 3 节「已知会静默失败的坑」尤其重要**，那些不报错、只让结论作废 |
...
**已完成 —— 探针物体集（交互原语库），10740 条 / 9046 成功（84.2%）**
...
**已完成 —— 擦拭 source（主任务），2700 条 / 2102 成功（77.9%）**
...
**已完成 —— 旋钮 source，2400 条 / 1707 成功（71.1%）**

四个销钉家族分别 480 / 299 / 480 / 448 条成功，`rim_only` 480 条**按设计全部失败**。
...
（安全上限 25 N 之内）时只转到 0.022 rad，销钉家族转到 2.34~2.76 rad。
