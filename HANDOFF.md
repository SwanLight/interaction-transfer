...
2. **泄漏检查第 4、8 条没有通过，是"本步做不完"**——它们要 envelope（S5 的产物）。
   **不得引用成通过。** 第 9 条（region 可推导性）做完了，但结论是**两个任务
   要降低对 region 的声称**：只看 effect 预测 region 热图的可解释方差，
   旋钮 0.344、擦拭 0.114、抽屉 0.050（作弊上界 0.560 / 0.132 / 0.175）。
   擦拭那 0.114 占了上界的 86%——effect 就是"哪片污渍被擦掉"，与 region
   近乎同义。按 Gate G 如实报告，不得调参掩盖（那是 P-11）。
