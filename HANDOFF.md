...
1. **逐点 10/90 分位数带不足以描述这批示教。** 名义带内率是 0.80，实测只有
   **0.24~0.68**，而且**按 cell 支持度分层后几乎不变**（旋钮 support≥10 仍 0.587），
   所以不是"支持太少"。同时带宽已达中位数量级的 0.64~3.86 倍——**带很宽而带内率
   仍低，说明问题不在带窄，在逐点边缘分位数这个形式本身**。`plan/04` §5.1 的
   `r_mech` 直接拿这条带当跟踪目标之前，必须先决定换 CQR 还是 functional
   simultaneous band。这正是 D-63 说的"冻结数据诊断显示需要时再加"的那个时刻。
