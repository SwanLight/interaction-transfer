...
| 旋钮成功轨迹 | ≥1000（≥4 策略家族） | 🟡 **未完成**，五个家族只有一个稳定通过（D-45） |
| **擦拭成功轨迹** | **≥1250（≥5 家族，含直擦）** | ✅ **2102 条 / 6 家族全部 ≥250**（共 2700，77.9%） |
| **探针物体集** | 每原语 ≥200 且 ≥2 个承载物体（D-41） | ✅ **8040 条 / 7023 成功**，15 条原语中 **10 条**两条判据都满足 |
| 一致性校验标记率 | <15% | ✅ 0.00%（脏帧比例） |
| 人工看过的轨迹视频 | ≥10 条 | ✅ 5 个家族各一条 + 逐帧接触轨迹核对 |

- [x] 校准集已独立划出（D-17）——55 条
- [x] 划分按 **episode** 而非帧（`pitfalls.md` P-10）
- [x] 策略分类器在**原始 source 动作**上准确率显著高于随机（多样性验收）——
      抽屉 0.775 / 随机 0.200（3.9×），擦拭 0.959 / 随机 0.167（5.8×），
      `tools/s3_diversity.py`
