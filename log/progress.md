...
### ✅ 抽屉 × 钩杆 Expert 已达标（2026-08-28）

`model_100.pt`（**只训了 100 轮**），256 并行 × 256+ episode：

| 环境 | 成功率 | 飞出边界 | 超时 | 终止开度 | 终止步数 |
|---|---:|---:|---:|---:|---:|
| 固定 | **100.0%** | 0% | 0% | 128.9 mm | 59.8 |
| 随机 | **100.0%** | 0% | 0% | 127.7 mm | 61.2 |

目标区间 100–160 mm，episode 上限 400 步。**Gate A 通过**（新门槛 D-32
只要求「能明确学会」，实测远超原来的 95%/85%）。

### 人工检查（`plan/06` §7）

录像 `out/s2/expert_drawer_hook.mp4`，4/4 成功。**画面确认策略在正经做任务**：
竖直主杆插进把手与面板之间的 45 mm 净空，沿 +X 拉，抽屉正常滑出。
无穿模、无瞬移、无异常抖动。

值得注意的是：**策略自己学到了和脚本验证时设计的同一个解法**
（主杆前面压把手背面，横钩只负责不碰撞），没有另辟蹊径去钻物理漏洞。

⚠️ 我最初只凭 100% 这个数就宣布 Gate A 通过，**跳过了人工看视频这一步**。
这个项目自己的规矩写着它是必经的，S1 也确实抓到过多次假 PASS。补上了。

### 为什么之前 239 轮不收敛

**根因只有一个：接触力尖峰 4384 N**（P-27）。

修复前后同一套代码、同一份奖励：

| | 峰值接触力 | 曲线 | 100 轮成功率 |
|---|---:|---|---:|
| 修复前 | 4383.6 N | +55 到 −256 全程乱甩 | 36.4% |
| 修复后 | 17.3 N | 单调上升到 +122 | **100%** |

之前猜的三个原因（奖励量纲 / 提前终止 / sign 悬崖）都改了，都不是主因。
**只有分项记录才定位到真凶。**
