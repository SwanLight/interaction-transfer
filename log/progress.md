...
| S3 | Source 脚本 + 数据集 + 探针物体集 | 🟡 抽屉 ✅ 探针集 ✅ | 2026-08-29 |
...
| 项 | 目标 | 实际 |
|---|---|---|
| **抽屉成功轨迹** | **≥450（≥3 策略家族）** | ✅ **741 条 / 5 家族**（共 800，成功率 92.6%） |
| **旋钮成功轨迹** | **≥1000（≥4 策略家族）** | ✅ **1707 条 / 4 家族各 ≥299**（共 2400，71.1%；`rim_only` 480 条按设计失败，D-46） |
| **擦拭成功轨迹** | **≥1250（≥5 家族，含直擦）** | ✅ **2102 条 / 6 家族全部 ≥250**（共 2700，77.9%） |
| **探针物体集** | 每原语 ≥200 且 ≥2 个承载物体（D-41） | ✅ **10740 条 / 9046 成功**，15 条原语 **15/15** 两条判据都满足（D-47） |
...
- [x] 策略分类器在**原始 source 动作**上准确率显著高于随机（多样性验收）——
      抽屉 0.775 / 随机 0.200（3.9×），擦拭 0.978 / 随机 0.167（5.9×），
...
ssh root@10.0.6.98 'cd /workspace/interaction_transfer && \
    PYTHONPATH=src /isaac-sim/python.sh tools/s3_verify_dataset.py /tmp/s3_drawer_full'
```

数据集在服务器 `/tmp/s3_drawer_full`（36 MB，800 个 npz + manifest），
按 D-23 不进版本控制。产物摘要在 `out/s3_source/`。

| 家族 | 条数 | 成功 | 开度 mm | 操作阶段脱手 | 受力方向对 | 接触在杆背面 |
|---|---:|---:|---:|---:|---:|---:|
| `pinch_center` 中央夹持 | 160 | 128 | 150.6 | 6.1% | 90.7% | 73.0% |
| `pinch_offset` 偏置夹持 | 160 | 140 | 147.5 | 6.6% | 93.3% | 78.2% |
| `hook_both` 双指纯勾 | 160 | 156 | 141.8 | 12.5% | 100.0% | 100.0% |
| `asym_primary` 非对称主副 | 160 | 157 | 140.9 | 10.8% | 100.0% | 100.0% |
| `single_finger` 单指 | 160 | 160 | 150.1 | 7.3% | 100.0% | 99.9% |
...
| 抽屉上的部位 | 占比 | | 板上的面 | 占比 |
|---|---:|---|---|---:|
| **把手横杆背面** | **90.22%** | | **工作面** | **100.00%** |
| 把手横杆正面（拇指） | 9.77% | | 背面 | 0.00% |
...
### 独立验收（`tools/s3_verify_dataset.py`，14 项全通过）
...
划分：train 411 / calibration 55 / in_distribution_test 82 /
unseen_physics_test 92 / unseen_strategy_test 160（留出家族 `asym_primary`）。
...
**9 个物体、8040 条 episode、7023 条成功（87.4%）。**

| 物体 | 条数 | 成功率 | | 物体 | 条数 | 成功率 |
|---|---:|---:|---|---|---:|---:|
| `block` | 2040 | 90.0% | | `dial` | 600 | 100% |
| `column` | 1200 | 68.8% | | `flap` | 420 | 100% |
| `roller` | 420 | 49.8% | | `ridge` | 840 | 100% |
| `slider` | 840 | 100% | | `slab` | 840 | 100% |
| `plunger` | 840 | 73.0% | | | | |

**15 条原语里 10 条同时满足"≥200 条成功"与"≥2 个承载物体"**，
逐条见 `out/s3_probe/coverage.txt`。未满足的五条（P7 roll、P8 twist、
P3 slide_push、P13 pinch_hold、P14 pinch_move）连同原因写在 `plan/03` §2.4.5
和 D-43，**没有从分类学里删掉**。
...
**2700 条 / 2102 成功（77.9%），六个家族全部 ≥250 条成功轨迹。**

| 家族 | 条数 | 成功 | 清除% | 力在区间% | 力N |
|---|---:|---:|---:|---:|---:|
| `tool_center` | 450 | 399 | 85.4 | 92.1 | 5.22 |
| `tool_offset` | 450 | 255 | 87.5 | 96.3 | 5.04 |
| `tool_tilt` | 450 | 352 | 79.6 | 86.2 | 4.78 |
| `tool_heavy_slow` | 450 | 292 | 68.0 | 82.9 | 4.53 |
| `tool_light_fast` | 450 | 423 | 89.9 | 94.2 | 5.27 |
| **`direct_wipe`** | 450 | **381** | 76.7 | 84.7 | 4.84 |

法向力 4.5~5.3 N（允许区间 3~8），工具滑脱 1.6~2.0 mm，擦拭阶段脱手 0.0%，
独立验收 15 项全通过。dirt 覆盖图见 `out/s3_wipe/dirt_tc.png`——
...
| 数据集 | 家族数 | 准确率 | 随机 | 提升 |
|---|---:|---:|---:|---:|
| 抽屉 | 5 | **0.775** | 0.200 | 3.9× |
| 擦拭 | 6 | **0.978** | 0.167 | 5.9× |
...
1. **旋钮 source**（D-45）：五个家族只有 `pin_push_dual` 稳定通过。
   已查明四个真 bug 并修掉，仍有一个未解释的现象——**位置控制的板压在刚体上时，
   实际接触力远高于"板刚度 × 指令干涉"，而 `pin_push_dual` 又与该估计吻合**。
   下一步应先用探针脚本量"干涉 → 接触力"的曲线，不要继续调增益。
2. **补齐三条冗余未满足的原语格**（`slide_push` / `pinch_hold` / `pinch_move`
   各只有一个承载物体，`twist` 数量略低）。
