...
| `r_region` | 落在指令 region 内的接触法向力占比**减去**区域外的占比 ∈[−1,+1]；没接触为 0。**这是四项里唯一能取正值的** |
...
**`r_region` 为什么必须能取正值**（D-84 / P-77，2026-08-31 实测补入）。第一版四项
**全部 ≤ 0**——三个 hinge 项在允许集合内恰好为零，`r_region` 写成"占比 − 1"也 ≤ 0
且没接触时恰好为 0。于是"什么都不做"的回报是 0，**任何真实接触都是负的**：
实测悬停 −0.43/步，而一段大致压对了的开环执行 −278/步。PPO 会稳稳地学会躲开物体，
**而训练曲线会一路上涨**。现在 `r_region` 是带符号的占比 ∈[−1,+1]，
且 `w_region > w_mech + w_mode`。四项都归一到 [−1,1]（hinge 项按**允许集合的宽度**
封顶），不再除以"成功示教上的标定量级"——那个小分母会把分布外的越界量放大成 −254。

⚠️ hinge 那条底线没有松动：mech/mode 落在标定集合内仍然恰好为 0，C4 没有被偷偷
实现成 C5。**这一条由 `tools/s6_smoke.py` 第六节实测把关**（"真正压进允许区域的
那些 env 的 reward" vs "悬停"，实测 +0.97 vs −0.01），不靠推理。
...
| entropy coef | 0.005（§9 原文没规定，理由见 D-87） |
| init noise std | 0.8（同上） |

**接线细节见 D-87**：观测是 TensorDict、自定义策略类要注进 runner 的模块命名空间、
**观测归一化必须关掉**（观测里有 command_index / bin_index 两个整数下标）、
critic 返回 `(N,1)` 不是 `(N,)`。

### 9.1 环境侧与"能不能学"直接相关的三个量（2026-08-31 实测补入）

| 量 | 值 | 为什么不能随便改 |
|---|---|---|
| `pos_limit` | **0.12 m**（逐轴） | 可达集合是立方体，对角线 √3·pos_limit。0.5 m 时对角线 0.87 m 超过失败门槛 0.75 m，**每条 episode 必然失败**，400 轮 reward 恒等于 −99.13（P-78）；0.25 m 时探索体积比目标大一个数量级，扩散出去就回不来（`diag/touching` 恒为 0） |
| `w_approach` / `approach_ref` | 1.0 / 0.26 m | 势函数必须**非负、远处为零**，且作用距离要覆盖整个可达集合。取负势时 `(γ−1)Φ` 奖励"离得越远越好"，实测 24 轮内策略跑到 0.465 m 外坐着不动（P-82 / D-89） |
| `fail_penalty_per_step` | 由 `s6_smoke.py` 实测定 | 失败终止必须显式惩罚，否则 hinge reward 下"立刻结束"与完美执行并列最优（D-83） |
