...
def _img(path, alt=""):
    """把 png 内嵌成 base64。S4 的产物是图不是录像，报告要能单文件带走。"""
    if not os.path.exists(path):
        return f"<p class=sub>（{alt} 缺失）</p>"
    with open(path, "rb") as h:
        return (f"<img alt='{alt}' style='width:100%;border-radius:8px;"
                f"border:1px solid #2a2f3a' src='data:image/png;base64,"
                f"{base64.b64encode(h.read()).decode()}'>")


def s4_section(s4_dir):
    """S4 段。**这一段要写得能读懂**——S4 没有录像，只有热图和一堆表，
    不解释清楚"它到底把什么变成了什么"，看图的人无从判断对错。"""
    P = ["<h2>S4 · 把示教变成物体中心的交互记录</h2>",
         "<p class=lead>这是整个工作的<b>转折点</b>：前面三步都在<b>产生</b>数据，"
         "S4 第一次把它<b>换了一种表达方式</b>。换完之后，记录里不再有"
         "「哪块板、以什么姿态、走了什么轨迹」，只剩下"
         "「<b>物体的哪片表面被碰了、以什么方式碰、受了多大的力、物体因此怎么变</b>」。</p>",

         "<div class=note><b>一句话说清它做了什么。</b>"
         "输入是一条示教：两块采集板（擦拭时还有一块黑板擦）在物体上蹭出来的几百帧接触数据。"
         "输出是同一条示教的<b>物体中心记录</b>——把所有接触体<b>合并</b>成一个接触集合，"
         "所有量转进<b>物体自己的坐标系</b>，再补上「接下来物体会怎么变」。"
         "<b>合并这一步是关键</b>：合并之后记录里没有「第几块板」这个概念，"
         "所以「换个执行器来做」在表示层面才是可能的——"
         "单板示教和双板示教产出的记录，字段与维度完全一样。</div>",

         "<h3 style='font-size:17px;margin:26px 0 8px'>记录里有什么（六个字段）</h3>",
         "<table><tr><th>字段</th><th>是什么</th><th>为什么需要它</th></tr>"
         "<tr><td><code>effect</code></td><td>物体接下来 1 秒会怎么变</td>"
         "<td>任务的目标。抽屉是拉开多少、旋钮是转过多少、擦拭是哪片污渍被擦掉</td></tr>"
         "<tr><td><code>region</code></td><td>力落在物体表面的哪些点上（热图）</td>"
         "<td>「该碰哪」。下面三张图画的就是它</td></tr>"
         "<tr><td><code>engage</code></td><td>每个接触点上力压进物体的方向</td>"
         "<td>「从哪个方向碰」。这一项相当于 KITE 的几何接触意图</td></tr>"
         "<tr><td><code>mode</code></td><td>每个接触点是贴着不动、在滑，还是分开了</td>"
         "<td>「怎么碰」。夹住拉与蹭着擦是完全不同的两件事</td></tr>"
         "<tr><td><code>mechanics</code></td><td>作用在物体上的合力/力矩，以及沿"
         "「物体能动的那个方向」的分量</td><td>「用多大力」。本工作主张这里只需要"
         "<b>方向与范围</b>，不需要精确复制示教的力</td></tr>"
         "<tr><td><code>phase</code></td><td>接近 / 建立接触 / 操作 / 松开</td>"
         "<td>用来把多条示教对齐。⚠️ 它是<b>标签</b>，不许当执行器的观测——"
         "那等于递给它一个任务专用状态机</td></tr></table>",

         "<h3 style='font-size:17px;margin:26px 0 8px'>region 热图：看图的方法</h3>",
         "<p class=lead>每行一个策略家族，三列是俯视 / 正视 / 侧视。"
         "<b>浅灰是物体表面的轮廓，颜色越暖表示那里累计受力越大。</b>"
         "要看的不是「好不好看」，而是<b>热点在不在该在的部位上</b>，"
         "以及<b>不同家族的热点是不是真的不一样</b>——后者是 S5 能否归纳出"
         "「允许集合」的前提。</p>"]

    for title, key, blurb in (
        ("抽屉", "drawer",
         "五个家族。前两行（非对称主副、双指纯勾）热点几乎全在把手横杆的<b>背面</b>——"
        ...
        ("擦拭", "wipe",
         "六个家族，含<b>不用工具直接以板面擦</b>那一档。热点铺在平面的目标区域上；"
         "持工具与直擦两种实现产生的记录<b>字段完全一样</b>——"
         "记录里没有任何「有没有工具」的痕迹，这是「表示与实现无关」的前提。"),
    ):
        png = os.path.join(s4_dir, f"region_{key}.png")
        P.append(f"<h4 style='font-size:15px;margin:20px 0 6px;color:#cbd3e1'>{title}</h4>")
        P.append(f"<p class=lead>{blurb}</p>")
        P.append(_img(png, f"{title} region 热图"))

    P += ["<h3 style='font-size:17px;margin:26px 0 8px'>怎么知道它没做错</h3>",
          "<p class=lead>S4 最危险的地方在于：<b>做错了不会报错</b>。"
          "接触归到了错误的部位、力的方向反了、贴着被判成在滑——"
          "成功率、接触力、脏帧比例全都正常。所以验收全部是"
          ...
          "这一项对上，说明法向定向、力作用在谁身上、力臂全都没错</td></tr>"
          "<tr><td>场景旋转不变</td><td>把整个场景绕世界原点转一个随机角度，"
          "物体系的表示按定义应当<b>逐位不变</b></td><td>region 索引差 0、mode 差 0、"
          "<b>力与力矩残差恰好为 0</b></td></tr>"
          "<tr><td>接触体数量无关</td><td>单板示教与双板示教的记录字段与维度</td>"
          "<td>完全一致</td></tr></table>",
          "<div class=note><b>这些检查不是形式主义——它们抓到了两个真 bug。</b>"
          "场景旋转检查逮到「相对速度算在世界系、法向在物体系」（P-53）；"
          "力臂普查逮到「接触体位姿是世界坐标、接触点是物体系」，制造了 8 米的假力臂"
          "（P-54）。<b>两次都零征兆</b>：逐帧数值、接触部位分布、动力学一致性、"
          "单元测试全部正常。现在提取器里有一条硬不变量——力臂超过 0.5 米直接报错——"
          "把这一类靠运气发现的错，变成了跑不起来的错。</div>",

          "<h3 style='font-size:17px;margin:26px 0 8px'>S4 顺带回答的两个问题</h3>",
          "<div class=note><b>一、装置能测到的那点东西，够不够用？</b><br>"
          "这是本方案的硬件前提——只测接触面的位姿和面上的触觉，不测手的动作。"
          "用已验收的记录当真值，模拟装置的观测模型再重建一遍，得到三条结论："
          "<b>（1）误差的时间结构比幅值更要命</b>——同样 0.5 mm，配准型偏置下"
          "stick/slide 判对率 0.82，每帧独立白噪声下只剩 0.08，因为滑移是靠位姿"
          "<b>差分</b>得到的；所以指标该写成「帧间稳定性」而不是「精度」，"
          "这恰好支持「粗跟踪 + 触觉配准」而不是高精度绝对跟踪。"
          "<b>（2）精度要求随功能接触区的尺寸缩放</b>——5 mm 误差下，"
          "销钉 Ø20 的 region 只剩 0.16，把手 140 mm 还有 0.72，"
          "擦拭区 190×290 mm 有 0.96；所以指标是「≤ 特征尺寸的 ~10%」。"
